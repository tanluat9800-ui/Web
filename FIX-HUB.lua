--[[ 
    REDZ HUB - BADDIES V18 (SAFE MODE - FIX NO UI)
    - Fix: Auto Detect GUI Parent (CoreGui or PlayerGui)
    - Fix: ZIndex Priority (Always on Top)
...
-- 1. HÀM TÌM NƠI CHỨA GUI AN TOÀN (FIX LỖI KHÔNG HIỆN)
...
--------------------------------------------------------------------------------
-- 2. HỆ THỐNG UI (ZINDEX CAO - LUÔN NỔI LÊN TRÊN)
--------------------------------------------------------------------------------

-- Xóa UI cũ
if ParentTarget:FindFirstChild("RedzV18") then ParentTarget.RedzV18:Destroy() end
local ScreenGui = Instance.new("ScreenGui", ParentTarget); ScreenGui.Name = "RedzV18"
ScreenGui.IgnoreGuiInset = true -- Full màn hình
ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Global -- Ép nổi lên trên cùng
ScreenGui.DisplayOrder = 9999 -- Cao hơn tất cả GUI của game

-- THÔNG BÁO KHỞI ĐỘNG (ĐỂ BIẾT SCRIPT ĐÃ CHẠY)
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "REDZ HUB V18",
    Text = "Script Started! Loading UI...",
    Duration = 3
})
...
function SendNotify(FeatureName, Status)
    spawn(function()
        ...
        local Title = Instance.new("TextLabel", NotifyFrame); Title.Size = UDim2.new(1, -20, 1, 0); Title.Position = UDim2.new(0, 15, 0, 0); Title.BackgroundTransparency = 1; Title.Text = FeatureName .. " [" .. (Status and "ON" or "OFF") .. "]"; Title.TextColor3 = Status and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100); Title.Font = Enum.Font.GothamBold; Title.TextSize = 14; Title.TextXAlignment = Enum.TextXAlignment.Left; Title.ZIndex = 100
...
--------------------------------------------------------------------------------
-- 3. LOADING SCREEN (ĐẢM BẢO KHÔNG BỊ KẸT)
--------------------------------------------------------------------------------
local LoadingHolder = Instance.new("Frame", ScreenGui); LoadingHolder.Size = UDim2.new(1, 0, 1, 0); LoadingHolder.BackgroundTransparency = 1; LoadingHolder.ZIndex = 1000
...
local LoadStroke = Instance.new("UIStroke", LoadBox); LoadStroke.Thickness = 2; LoadStroke.Color = Color3.fromRGB(255, 0, 0)
local LoadTitle = Instance.new("TextLabel", LoadBox); LoadTitle.Text = "REDZ HUB V18 FIXED"; LoadTitle.Size = UDim2.new(1, 0, 0, 40); LoadTitle.TextColor3 = Color3.fromRGB(255, 255, 255); LoadTitle.Font = Enum.Font.GothamBlack; LoadTitle.TextSize = 18; LoadTitle.BackgroundTransparency = 1
...
--------------------------------------------------------------------------------
-- 4. MAIN UI (NEON RAINBOW)
--------------------------------------------------------------------------------
local ToggleBtn = Instance.new("ImageButton", ScreenGui); ToggleBtn.Visible = false; ToggleBtn.Size = UDim2.new(0, 50, 0, 50); ToggleBtn.Position = UDim2.new(0.1, 0, 0.3, 0); ToggleBtn.BackgroundColor3 = Color3.fromRGB(10, 10, 10); ToggleBtn.BorderSizePixel = 0; ToggleBtn.ZIndex = 100
local ToggleCorner = Instance.new("UICorner", ToggleBtn); ToggleCorner.CornerRadius = UDim.new(1, 0)
local ToggleStroke = Instance.new("UIStroke", ToggleBtn); ToggleStroke.Thickness = 3 
local ToggleText = Instance.new("TextLabel", ToggleBtn); ToggleText.Size = UDim2.new(1,0,1,0); ToggleText.BackgroundTransparency = 1; ToggleText.Text = "R"; ToggleText.TextColor3 = Color3.fromRGB(255, 255, 255); ToggleText.Font = Enum.Font.FredokaOne; ToggleText.TextSize = 28; ToggleText.ZIndex = 101

local Main = Instance.new("Frame", ScreenGui); Main.Size = UDim2.new(0, 450, 0, 320); Main.Position = UDim2.new(0.5, -225, 0.5, -160); Main.BackgroundColor3 = Color3.fromRGB(10, 10, 10); Main.Visible = false; Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 10); Main.ZIndex = 90
...
function AddSlider(Page, Key, Flag, Min, Max, Default)
    ...
    SliderBG.MouseButton1Down:Connect(function()
        local Mouse = Players.LocalPlayer:GetMouse(); local MoveConnection, ReleaseConnection
        local function Update() local Percent = math.clamp((Mouse.X - SliderBG.AbsolutePosition.X) / SliderBG.AbsoluteSize.X, 0, 1); local Value = math.floor(Min + (Max - Min) * Percent); getgenv().Settings[Flag] = Value; Label.Text = "  " .. GetText(Key) .. ": " .. Value; Fill.Size = UDim2.new(Percent, 0, 1, 0) end
        Update(); MoveConnection = Mouse.Move:Connect(Update); ReleaseConnection = UserInputService.InputEnded:Connect(function(Input) if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then MoveConnection:Disconnect(); ReleaseConnection:Disconnect() end end)
    end)
end
...
-- ITEMS
local Tab2 = AddTab("Tab2"); local Tab3 = AddTab("Tab3"); local Tab4 = AddTab("Tab4")
...
AddSlider(Tab2, "Size", "HitboxSize", 2, 45, 15) -- MAX 45
...
spawn(function()
    ...
    wait(0.2); LoadingHolder:Destroy(); ToggleBtn.Visible = true
end)
...
RunService.RenderStepped:Connect(function()
    if getgenv().Settings.AimLock then
        ...
        if Nearest then Camera.CFrame = CFrame.new(
